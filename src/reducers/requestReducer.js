...
export const LOAD_REQUESTS_SUCCESS = requestAction(
  'LOAD_REQUESTS_SUCCESS'
);
export const CREATE_REQUEST_SUCCESS = requestAction(
  'CREATE_REQUESTS_SUCCESS'
);
export const CREATE_REQUEST_FAILED = requestAction(
  'CREATE_REQUEST_FAILED'
);
export const REMOVE_REQUEST_SUCCESS = requestAction(
  'REMOVE_REQUEST_SUCCESS'
);
export const SAVED_FORM_ERRORS = requestAction(
  'SAVED_FORM_ERRORS'
);
...
const requestReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case LOAD_REQUEST_UTILS_SUCCESS:
      return {...state, requestStatusesList: action.requestUtils.requestStatuses, requestTypesList: action.requestUtils.requestTypes};
    case SAVED_FORM_ERRORS:
      return {...state, errors: action.errors };
    case REMOVE_REQUEST_SUCCESS:
      return {...state, requestsList: _.omit(state.requestsList, action.requestId)};
    case CHANGE_REQUEST_STATUS_SUCCESS:
      return {...state, requestsList: {...state.requestsList, [action.updatedRequest.id]: action.updatedRequest}};
      default:
      return state;
  }
};
